...
let data = "请稍等"
let instructions= "该页面生成课表的订阅地址。在上方的“课前提醒”中，你可以设置课前提醒的时间差。将对应的同步地址复制到剪切板后，你可以使用浏览器下载并导入日历文件到手机自带的日历软件中，或直接将该同步地址复制到日历软件（如小米日历）里。"
...
Page({
   ...
   */
  data: {
    data,instructions,time,choosed_time,calendar
  },
  // 导航栏函数
  handlerGohomeClick,
  handlerGobackClick,
  ...
  setUrl(){
    ...
    let time  
    _this.data.choosed_time =="不提醒" ? time=0:time=_this.data.choosed_time 
    console.log(time)
    time = time*60
