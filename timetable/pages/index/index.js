...
refresh() {
  ...
  bindchangeday(e) {
    ...
    } else if (_this.data.choosedday.week == 0) {
      _this.data.this_week--;
  ...
  getName(e) {
    ...
    for (let i = 0; i < newlist.length; i++) {
      ...
      time = time + section - 1
      newlist[i].weekday = section + (
        time == section ? '周 · 周' : section + '-' + time + '周·周'
      ) + newlist[i].days + '(' + newlist[i].table_time + ')';
      newlist[i].time = time;
    }
    newlist.map(el => {
    ...
    _this.animation.translate(0, _this.data.isShow_detail ? 450 : -450).step()
    _this.setData({
