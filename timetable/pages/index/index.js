...
Page({
  ...
  SendData(e) {
    let _this = this;
    let urlCalendar = `${app.globalData.commonUrl}${calendarSuffix}`;
    let urlSchedule = `${app.globalData.commonUrl}${scheduleSuffix}`;
    ...
    schedule.then((res) => {
      ...
      _this.setData({ table: dataSchedule });
  ...
  setTimetable(year, semester, header) {
    ...
    timeTable
      .then((res) => {
        ...
        dayCourse = _this.binary(
          dataTimetable,
          _this.data.thisWeek,
          _this.data.choosedDay
        );  //日课表
        weekCourse = _this.binaryWeek(dataTimetable, _this.data.thisWeek);  //周课表
        _this.setData({
          courseData: dayCourse,
          courseWeek: weekCourse,
        });
        wx.setStorageSync("timetable_list", dataTimetable);//todo
        wx.hideLoading();
    ...
    for (var j = 0; j < e.length; j++) {
      for (var i = 0; i < e[j].table.length; i++) {
        if (e[j].table[i] === 1 && y == 0) {
          ...
          ? e[j].tables[0] = table[e[j].campus][e[j].place.substring(0,2)][i][0]
          : e[j].tables[0] = table[e[j].campus]["default"][i][0];
          y = 1;
        } else if (e[j].table[i] === 0 && y == 1) {
          table[e[j].campus][e[j].place.substring(0,2)] != undefined
          ? e[j].tables[1] = table[e[j].campus][e[j].place.substring(0,2)][i - 1][1]
...
refresh() {
  ...
  ? wx.showModal({
    showCancel: false,
    content: '一般情况下，小风筝课表会在一定天数后自动刷新。' +
  ...
  binary(list, thisWeek, day) {
    ...
    newlist = list.filter((el) => el.weeks[thisWeek - 1] == "1");
  ...
  sliderchange(e) {
    ...
    let course_week = _this.binaryWeek(_this.data.list, _this.data.thisWeek);
  ...
  bindchange(e) {
    ...
    switch (index) {
      case 1:
        page == 0 ? thisWeek++ : thisWeek--;
        break;
      case 2:
        page == 1 ? thisWeek++ : thisWeek--;
        break;
      case 0:
        page == 2 ? thisWeek++ : thisWeek--;
        break;
    }
    page = index;
    if (thisWeek <= 0) {
  ...
  bindchangeday(e) {
    ...
    switch (index) {
      case 1:
        page_day == 0 ? this_day++ : this_day--;
        break;
      case 2:
        page_day == 1 ? this_day++ : this_day--;
        break;
      case 0:
        page_day == 2 ? this_day++ : this_day--;
        break;
    }
    page_day = index;
    ...
    if (_this.data.choosedday.week == 0) {
      _this.data.thisWeek--;
      if (_this.data.thisWeek == 0) {
        _this.data.thisWeek = 1;
      }
      _this.changeTime(_this.data.toschool, _this.data.thisWeek);
    }
    let course_data = _this.binary(
  ...
  getName(e) {
    ...
    for (let i = 0; i < newlist.length; i++) {
      newlist[i].table_time =
        newlist[i].table_time[0] + "~" + newlist[i].table_time[1] + "节";
    ...
    let result = [
      { courseName, tables, teacher, weekday, place, courseId, dynClassId },
    ];
