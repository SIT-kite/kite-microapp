...
requirePlugin("echarts").echarts = echarts;
...
/* (async () => {
  requirePlugin("echarts").echarts = await import("./echarts.min.js");
})().then( () => */Page({
	...
	data: {
		...
		option: {
			title: { text: "电费使用情况" },
			tooltip: { trigger: "axis", formatter: '在{b0}\n消费{c0}' },
			series: [{
				data: [],
				type: "line",
				markLine: {
					data: [{ type: 'average', name: '平均值', symbol: "none" }]
				}
			}],
			xAxis: {
				data: [],
